...
export async function GET(
  ...
  const licensePlate = parseInt(resolvedParams.id);

  const handler = withAuth(async (authRequest: AuthenticatedRequest, user) => {
    try {
      // Verificar que el license plate sea válido
      if (isNaN(licensePlate)) {
      ...
      // Obtener el transfer con sus detalles
      const transfer = await prisma.transfer.findUnique({
        where: { license_plate: BigInt(licensePlate) },
        select: {
          license_plate: true,
          availability: true,
          make: true,
...
export async function PUT(
  ...
  const handler = withAuth(async (authRequest: AuthenticatedRequest, user) => {
    try {
      ...
      // Verificar que el transfer existe
      ...
      // Actualizar el transfer (no se puede actualizar license_plate ya que es la PK)
