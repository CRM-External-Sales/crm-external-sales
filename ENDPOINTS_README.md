...
- `GET /api/reservations` - Listar con filtros (`date`, rango `dateFrom`/`dateTo`, `state`, `transfer_id`, búsqueda `q`, paginación). **Admin y agente** ven el **mismo listado global** (no se filtra por `employee_user`). El rol **customer** no usa este endpoint (`withRole("agent")`).
- `GET /api/reservations/:id` - Detalle. **Admin** y **agente** ven cualquier reserva. **Customer** solo si `employee_user` coincide con su ID.
- `PUT /api/reservations/:id` - **Admin:** puede modificar `people`, `tour_id`, `transfer_id`, `date`, `time`, `hotel_reservation`, `note` y recalcula montos según corresponda; además **solo** puede fijar `state` a cancelación: `state: "cancelled"` (con `cancellation_reason` obligatorio vía `UpdateReservationSchema`). No se acepta otro `state` en el cuerpo. Valida franja al guardar salvo que se esté **cancelando** (al sumar cupo, excluye la fila actual). **Agente:** puede **anular cualquier reserva** con motivo (misma política de plazo/penalidad). **La nota** solo la modifican **el agente creador de la fila** (`employee_user`) o **admin**.
